...
class _CreateYourShopPageState extends State<CreateYourShopPage> {
  ...
  void _handleContinue() async {
    ...
    // ── 3. FIRESTORE REPEATED NAME CHECK ──
    ...
    try {
      ...
      if (nameCheck.docs.isNotEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('The shop name "$shopName" is already taken. Please choose another!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error verifying name availability: $e')),
        );
      }
      return;
    }
